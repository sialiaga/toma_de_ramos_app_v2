...
def index(request):
    return render(request, 'toma_de_ramos_app/home.html')
        ...
        datos = [
            {
                ...
                'INICIO': registro.INICIO if registro.INICIO else '',
                'FIN': registro.FIN if registro.FIN else '',
                'TIPO': registro.TIPO,
...
def api_cursos(request):
    if request.method == 'GET':
        registros = tablaRamos.objects.values('NRC', 'TITULO', 'PROFESOR', 'CREDITO').distinct()          
